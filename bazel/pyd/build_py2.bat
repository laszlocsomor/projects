...
cl.exe /c lib_alice\alice.cc /Fo_outs\alice.obj /Ic:\python_27_amd64\files\include
@if "%errorlevel%" neq "0" (echo FAILED >&2 & exit /b 1)

link.exe /OUT:_outs\alice.pyd /IMPLIB:_outs\alice.lib /DLL /MACHINE:X64 /LIBPATH:C:\python_27_amd64\files\libs _outs\alice.obj _outs\b.lib
@if "%errorlevel%" neq "0" (echo FAILED >&2 & exit /b 1)
